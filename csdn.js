 ...
 *
 * 这个编辑器（StackEdit 那一系，<pre class="editor__inner" contenteditable>）里躺的
 * **就是 Markdown 源码本身**，不是富文本、也不是什么内部模型的投影。所以不需要
 * md → HTML，不需要按图片把正文切成一段段顺着贴，也不用去够框架的内部状态。
 ...
 *
 * 于是顺序是「一张张传（只为拿 URL）→ 最后整篇写一次」，而不是边传边往正文里贴：
 ...
 * 但它的服务器够不着的地方就抓不到——实测 pbs.twimg.com（X 的图床）那条永远停在
 * `[外链图片转存中...(img-xxx)]`，连图片语法都不是了，发出去就是一行废字。
 * 墙和防盗链是按「谁在发请求」算的：浏览器带着你的网络和身份，CSDN 的服务端没有。
 * 所以我们能在浏览器里 fetch 到的就自己传；fetch 不到的（多半是对方没给 CORS 头）
 * 就原样留着交给 CSDN 转存，它服务端往往反而抓得到——两条路互补，不是二选一。
 *
 * 三个实测出来的坑，改代码前先看一眼：
 ...
 *
 * 这个文件是整个扩展的全部逻辑，不依赖任何其他脚本，也不用任何 chrome.* API。
 */
(function () {
  'use strict';
